...
- Discord: Our primary communication platform. Select `WATcloud Incoming` if you are new. Team leads decide when to grant `WATcloud` and `Core Member`, based on the amount and quality of work you've completed.
- Google Workspace: Your `@watonomous.ca` email account. Your 1Password invite link is sent to this inbox.
...
- Sentry: Our error tracking tool. Select `WATcloud` to get access to WATcloud projects.
- Additional Comments: We have moved away from onboarding projects for now. Admission is task-oriented, with an evaluation period of half a term in most cases.
...
WATcloud hosts weekly meetings. The meeting time is decided at the start of each term by existing and incoming members; for the Fall 2026 term it is **Monday at 8:00 PM Eastern Time**. These meetings take place on <Link href={`https://discord.gg/${websiteConfig.discord_invite_code}`}>Discord</Link> in the `General 1` voice channel. Sometimes, we may also have meetings in-person in the server room (`CPH-3667`) to perform hardware maintenance.
