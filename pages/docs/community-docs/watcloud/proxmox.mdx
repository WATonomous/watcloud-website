...
If changes to another VM are shown, someone probably forgot to commit their changes.
...
This will allow the cluster switch to receive the new hostname and assign a temporary `.watocluster.local` domain for initial provisioning.
...
This is because there is a circular reference--some parts of the repo (namely DNS management) depend on the microk8s configuration,
...
When this happens, you can interrupt the process (`Ctrl+C`) and replace the `ansible_hosts` entry in the configuration with the new domain provisioned earlier.
