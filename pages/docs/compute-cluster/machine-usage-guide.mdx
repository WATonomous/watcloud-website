...
Most machines in the cluster come with standard workstation hardware that includes CPU, RAM, GPU, and storage[^machine-specs]. In special
...
We try to keep the machines lean and generally refrain from installing software that makes sense for rootless installation or running in
