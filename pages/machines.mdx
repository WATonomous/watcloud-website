...
There are
[{pluralizeWithCount(machineInfo.machines['slurm_compute_nodes'].length, "compute node")}](#compute-nodes)
and [{pluralizeWithCount(machineInfo.machines['slurm_login_nodes'].length, "login node")}](#login-nodes)
